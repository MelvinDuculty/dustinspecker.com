...
date: 2020-07-06T00:59:28Z
lastmod: 2020-07-06T00:59:28Z
draft: false
categories:
  - development
tags:
...
Docker uses a couple of techniques including a proxy that binds port `8080` in the host network
namespace and makes requests to a port in the container's network namespace.

This post focuses on the other technique Docker uses, iptables, which can be used to forward
requests to a port in the host network namespace to an IP address and port residing in another
network namespace.
...
Let's enable IP forwarding by running:
...
I'm stubborn and don't like admitting defeat to computers, so I wanted to understand how to use
...
our port forwarding on the local network. Run:
